...
object DeletePostConsumer extends App with Config with JsonMappings {


  val consumer = new KafkaConsumer(kafkaDeletePostTopic)
  ...
  while (true) {
    consumer.read() match {
      case Some(message) =>
        ...
        PostsDao.delete(key.toInt)
          .onComplete {
            case scala.util.Success(e) =>
              println (s"Delete Post Consumer Success ${e}")
              if( e == 1)
                consumer.commitOffset(offsetMap)

            case Failure(e) =>
              println(s"Delete Post Consumer Failure ${e}")
          }
